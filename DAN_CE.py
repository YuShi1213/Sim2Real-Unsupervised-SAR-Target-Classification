...
def train(model):
    ...
    for i in range(1, iteration+1):
        ...
        optimizer.zero_grad()
        src_pred,src_pred_CE = model(src_data)
...
def test(model,proxcy):
    ...
    with torch.no_grad():
        for tgt_test_data, tgt_test_label in tgt_test_loader_test:
            ...
            tgt_pred,tgt_pred_CE= model(tgt_test_data)
            propobality = F.softmax(tgt_pred_CE, dim=1)
            
            pred = tgt_pred_CE.data.max(1)[1] # get the index of the max log-probabilitypred = distances.data.min(1)[1] # get the index of the max log-probability
